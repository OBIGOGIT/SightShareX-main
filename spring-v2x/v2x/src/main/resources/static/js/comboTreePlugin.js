...
;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if (this.options.isMultiple){
      if(typeof(sourceItem.singlemode) == 'undefined')
        ...
        itemHtml += '<span data-id="' + sourceItem.id + '" mode="' + sourceItem.singlemode + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" />' : '') + sourceItem.title + '</span>';
      }
    }
    else
  ...
  // SELECTION FUNCTIONS
  ComboTree.prototype.selectMultipleItemDel = function(ctItem){
  ...
  ComboTree.prototype.multiItemClick = function (ctItem) {
    if($(ctItem).children('input[type="checkbox"]').prop("disabled") == true) return;
    ...
    if(ctItem.getAttribute('mode') == 'true') {
      var itemsList = $(ctItem).parent().siblings();
      var ctItemState =  $(ctItem).children('input[type="checkbox"]').prop("checked");
      for(var i = 0; i< itemsList.length; i++) {
        if(ctItemState) {
          $(itemsList[i]).children().children('input[type="checkbox"]').prop("disabled", true);
          if($(itemsList[i]).hasClass('ComboTreeItemParent')) {
            var subMenu = $(itemsList[i]).children('ul').first().find('input[type="checkbox"]');
            subMenu.each(function() {
              var $input = $(this)
          ...
          $(itemsList[i]).children().children('input[type="checkbox"]').prop("disabled", false);
          if($(itemsList[i]).hasClass('ComboTreeItemParent')) {
            var subMenu = $(itemsList[i]).children('ul').first().find('input[type="checkbox"]');
            subMenu.each(function() {
              var $input = $(this)
      ...
      let itemElem = $("#" + this.comboTreeId + 'Li' + $(ctItem).attr("data-id"));
      if($(itemElem).find("input").prop('checked')) {
        this.clearSelectionIdx(ctItem);
      }
      else {
        this.clearSelection();
      }
  ...
  ComboTree.prototype.clearSelectionIdx = function(ctItem) {

    for (i=0; i<this._selectedItems.length; i++) {
      if(this._selectedItems[i].id == $(ctItem).attr("data-id")) {
        this._selectedItem = {
          id: $(ctItem).attr("data-id"),
          title: $(ctItem).text()
        };
