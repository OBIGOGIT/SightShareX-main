var showProfile = function(uname, email) {
	...
	axios.post( '/common/email/dec', params )
	.then( function (response) {
		Log.d( 'response.data : ', response.data );
		if ( response.data.result == 'ok' ) {
			getID('pf-email').value = response.data.email;
		}else{
			getID('pf-email').value = "";
		}
	})
    .catch( function (error) {
    	 Log.d('showProfile() -> error=', error);
    });
...
var initPage = function() {	
	let pw = getID('pf-pw'); // pw flag init
	pw.lowChk = false;
	pw.upChk = false;
	pw.numChk = false;
	pw.lenChk = false;
...
getID('btnPfModalSave').onclick = function(e) {
	...
	//----------------------------------
	// 제약 조건 체크
	let x = document.getElementsByClassName("pf-help-block");
	let i;
