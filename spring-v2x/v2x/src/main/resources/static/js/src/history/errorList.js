	...
	.then( function(response) {
		...
		for ( let i = 0 ; i < len ; i++ ) {
			...
			for ( let k = COL_START ; k < key.length ; k++ ) {
				...
				} else if ( key[k] == 'Message' ) {
					td += "<td class='col-xs-6 text' title='%s' style='cursor: pointer;' onclick='showMsg(%s);'><span>%s</span></td>".sprintf( result[i][key[k]], JSON.stringify(result[i][key[k]]), result[i][key[k]] );
				} else {
