...
<script>
    ...
    const testArr = function() {
        ...
        // 사용하는 방법:
        const vehicleLat = mapConfig.hlv.origin[1];
        const vehicleLng =   mapConfig.hlv.origin[0];
        const vehicleHeading = 41;  // 북쪽을 기준으로 차량의 진행 방향
        const distances = {
            front: 0.02,  // 전방 거리 (20 미터)
            back: 0.02,   // 후방 거리 (20 미터)
            left: 0.004,  // 좌측 거리 (15 미터)
            right: 0.004  // 우측 거리 (15 미터)
        };

        const polygonPoints = calculateVehiclePolygon(vehicleLat, vehicleLng, vehicleHeading, distances);

        map.addSource('hlv-polygon', {
            'type': 'geojson',
            'data': {
                'type': 'Feature',
                'properties': {},
                'geometry': {
                    'type': 'Polygon',
                    'coordinates': [polygonPoints]  // 'coordinates'는 2차원 배열을 포함해야 합니다.
                }
            }
        });
        map.addLayer({
            'id': 'hlv-polygon-fill',
            'type': 'fill',  // 폴리곤 내부를 채우기 위한 레이어 타입
            'source': 'hlv-polygon',
            'paint': {
                'fill-color': 'blue',
                'fill-opacity': 0.3
